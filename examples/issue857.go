...
func main() {
	fmt.Printf("1\n")
	defer fmt.Printf("winsparkle.Cleanup()\n")
	defer winsparkle.Cleanup()
	winsparkle.SetAppcastURL("https://winsparkle.org/example/appcast.xml")
	fmt.Printf("2\n")
	winsparkle.SetAppDetails("winsparkle.org", "WinSparkle Go Example1", "1.0")
	fmt.Printf("3\n")
	winsparkle.SetAutomaticCheckForUpdates(true)
	fmt.Printf("4\n")
	winsparkle.SetDSAPubPEM(dsaPublicKey)
	fmt.Printf("5\n")
	...
	w := webview.New(false)
	fmt.Printf("7\n")
	defer fmt.Printf("w.Destroy()\n")
	defer w.Destroy()
}

var dsaPublicKey = `-----BEGIN PUBLIC KEY-----
